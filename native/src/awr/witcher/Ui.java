...
public final class Ui {
    ...
    public static LinearLayout column(Context c){LinearLayout v=new LinearLayout(c);v.setOrientation(LinearLayout.VERTICAL);v.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);return v;}
    public static LinearLayout row(Context c){LinearLayout v=new LinearLayout(c);v.setGravity(Gravity.CENTER_VERTICAL);v.setLayoutDirection(View.LAYOUT_DIRECTION_RTL);return v;}
    public static TextView button(Context c,String s,View.OnClickListener action){TextView v=text(c,s,15,true);v.setGravity(Gravity.CENTER);v.setMinHeight(dp(c,48));v.setPadding(dp(c,16),dp(c,10),dp(c,16),dp(c,10));clickable(v,surface(c),12);v.setOnClickListener(action);return v;}
    public static ImageView image(Context c,String url,int radius){return image(c,url,radius,false);}
    public static ImageView hitvImage(Context c,String url,int radius){return image(c,url,radius,true);}
    private static ImageView image(Context c,String url,int radius,boolean hitv){ImageView v=new ImageView(c);v.setScaleType(ImageView.ScaleType.CENTER_CROP);v.setBackground(rounded(c,surface(c),radius));v.setClipToOutline(true);load(v,url,hitv);return v;}
    public static void load(ImageView view,String url){load(view,url,false);}
    private static void load(ImageView view,String url,boolean hitv){view.setTag(url);if(url==null||!(url.startsWith("https://")||url.startsWith("http://")))return;Bitmap cached=CACHE.get(url);if(cached!=null){view.setImageBitmap(cached);return;}WeakReference<ImageView> ref=new WeakReference<>(view);IMAGES.execute(()->{HttpURLConnection connection=null;try{URL target=new URL(url);connection=(HttpURLConnection)(hitv?HitvNet.open(target):target.openConnection());connection.setConnectTimeout(12000);connection.setReadTimeout(hitv?16000:15000);connection.setRequestProperty("User-Agent",hitv?"Mozilla/5.0 (Linux; Android 15) AppleWebKit/537.36 Chrome/127.0 Mobile Safari/537.36":"Mozilla/5.0 (Android)");ByteArrayOutputStream data=new ByteArrayOutputStream();try(InputStream in=connection.getInputStream()){byte[] buffer=new byte[16384];int n;while((n=in.read(buffer))!=-1){data.write(buffer,0,n);if(data.size()>10*1024*1024)throw new IOException("Image too large");}}byte[] bytes=data.toByteArray();BitmapFactory.Options o=new BitmapFactory.Options();o.inJustDecodeBounds=true;BitmapFactory.decodeByteArray(bytes,0,bytes.length,o);o.inSampleSize=1;while(o.outWidth/o.inSampleSize>1200||o.outHeight/o.inSampleSize>1600)o.inSampleSize*=2;o.inJustDecodeBounds=false;Bitmap bitmap=BitmapFactory.decodeByteArray(bytes,0,bytes.length,o);if(bitmap==null)return;CACHE.put(url,bitmap);MAIN.post(()->{ImageView v=ref.get();if(v!=null&&url.equals(v.getTag()))v.setImageBitmap(bitmap);});}catch(Exception ignored){}finally{if(connection!=null)connection.disconnect();}});}
    public static final class Icon extends View {
        private final String name;private int color;private final Paint p=new Paint(3);
        public Icon(Context c,String name,int color){super(c);this.name=name;this.color=color;setImportantForAccessibility(IMPORTANT_FOR_ACCESSIBILITY_NO);}public void tint(int value){color=value;invalidate();}
        protected void onDraw(Canvas c){super.onDraw(c);float size=Math.min(getWidth()-getPaddingLeft()-getPaddingRight(),getHeight()-getPaddingTop()-getPaddingBottom());c.save();c.translate((getWidth()-size)/2f,(getHeight()-size)/2f);c.scale(size/24f,size/24f);p.setColor(color);p.setStrokeWidth(1.8f);p.setStrokeCap(Paint.Cap.ROUND);p.setStrokeJoin(Paint.Join.ROUND);p.setStyle(Paint.Style.STROKE);Path path=new Path();switch(name){
