...
public final class DramaActivity extends Activity {
    ...
    private void render(){
        ...
        setContentView(root);switch(kind){case "search":showSearch();break;case "detail":showDetail();break;case "actor":showActor();break;case "seasons":showSeasons();break;case "sources":showSources();break;default:showCatalog();}
    }
    ...
    private void renderDetail(JSONObject item){
        ...
        if(!channel){LinearLayout cast=Ui.column(this);content.addView(cast);load("role/by/poster/"+item.optInt("id")+"/",cast,value->{JSONArray actors=Api.array(value);if(actors.length()>0){heading(cast,"فريق العمل");Cards.actors(this,cast,actors,actor->{JSONObject n=make("actor");put(n,"item",actor);put(n,"title",Api.label(actor));navigate(n);});}});}
    }
    private void showActor(){JSONObject actor=page.optJSONObject("item");if(actor==null)return;load("movie/by/actor/"+actor.optInt("id")+"/",content,value->{JSONArray items=Api.array(value);if(items.length()==0)message(content,"لا توجد أعمال متاحة لهذا الممثل حالياً.");else Cards.grid(this,content,items,false,this::detail);});}
    private void showSeasons(){
        JSONObject show=page.optJSONObject("item");load("season/by/serie/"+show.optInt("id")+"/",content,value->{JSONArray seasons=Api.array(value);if(seasons.length()==0){message(content,"لم تُضف حلقات لهذا العمل بعد.");return;}
            int rememberedSeason=page.optInt("seasonIndex",0);if(rememberedSeason<0||rememberedSeason>=seasons.length())rememberedSeason=0;final int openSeason=rememberedSeason;
            for(int i=0;i<seasons.length();i++){
                final int seasonIndex=i;JSONObject season=seasons.optJSONObject(i);if(season==null)continue;JSONArray episodes=Api.array(season.optJSONArray("episodes"));LinearLayout list=Ui.column(this);TextView head=Ui.button(this,Api.label(season)+"  ·  "+episodes.length()+" حلقة",v->{boolean opening=list.getVisibility()!=View.VISIBLE;list.setVisibility(opening?View.VISIBLE:View.GONE);if(opening)put(page,"seasonIndex",seasonIndex);});LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,-2);hp.topMargin=Ui.dp(this,12);content.addView(head,hp);content.addView(list);if(seasonIndex!=openSeason)list.setVisibility(View.GONE);
    ...
    private void serverRows(LinearLayout parent,JSONArray servers,JSONObject item,boolean download){
        int shown=0;for(int i=0;i<servers.length();i++){
            JSONObject source=servers.optJSONObject(i);if(source==null)continue;String kind=source.optString("kind");if(!kind.equals("both")&&!kind.equals(download?"download":"play"))continue;
            if(download&&("embed".equals(source.optString("type"))||"youtube".equals(source.optString("type"))))continue;shown++;
            ...
            row.setOnClickListener(v->{if(!Api.publicAccess(source.optString("premium"))){restricted();return;}put(source,"_channel",Api.channel(item));Media.open(this,source,Api.label(item),download);});LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1,-2);lp.topMargin=Ui.dp(this,10);parent.addView(row,lp);
