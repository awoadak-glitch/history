...
public final class WitcherTabs extends LinearLayout {
    ...
    public static final String[] LABELS={"الأنمي","المسلسلات","الأفلام","القنوات"};
    private static final String[] ICONS={"anime","series","movies","channels"};
    ...
    private void render(){
        ...
        for(int i=0;i<4;i++){
            ...
            TextView label=Ui.text(c,LABELS[i],12,active);label.setTextColor(active?Ui.ACCENT:Ui.muted(c));label.setGravity(Gravity.CENTER);
            LayoutParams t=new LayoutParams(-1,-2);t.topMargin=Ui.dp(c,3);item.addView(label,t);
            LayoutParams lp=new LayoutParams(0,Ui.dp(c,56),1);lp.setMargins(Ui.dp(c,4),Ui.dp(c,4),Ui.dp(c,4),Ui.dp(c,4));addView(item,lp);
            item.setOnClickListener(v->{if(index!=selected && listener!=null)listener.select(index);});
        }
    }
}
