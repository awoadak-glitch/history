...
public final class ModuleRuntime {
    ...
    public static void open(Activity activity){
        ...
        IO.execute(()->{
            try { prepare(); activity.runOnUiThread(()->{
                ...
                catch(Throwable e){error(activity,"تعذر بدء عالم المصادر: "+e.getClass().getSimpleName());}
                finally {if(!activity.isFinishing())wait.dismiss();}
            }); } catch(Throwable e){activity.runOnUiThread(()->{if(!activity.isFinishing()){wait.dismiss();error(activity,"تعذر تجهيز عالم المصادر: "+e.getClass().getSimpleName());}});}
        });
    }
    ...
    static synchronized void prepare()throws Exception{
        ...
        module=new File(root,"sources.apk");
        if(!module.isFile()||!digest(module).equals(ModuleConfig.SHA256)){
            if(module.exists()&&!module.delete())throw new IOException("Stale module");
            try(InputStream in=host.getAssets().open("atheer/sources.apk");FileOutputStream out=new FileOutputStream(module)){
                // Android 14: declare the dynamic-code file read-only before writing its bytes.
                if(!module.setReadOnly())throw new IOException("Read-only code");copy(in,out);
            }
            if(!digest(module).equals(ModuleConfig.SHA256)){module.delete();throw new IOException("Module checksum");}
        }
        ...
        int cookie=(Integer)AssetManager.class.getMethod("addAssetPath",String.class).invoke(nextAssets,module.getAbsolutePath());
    ...
    static final class FeatureLoader extends DexClassLoader{
        ...
        @Override protected Class<?> loadClass(String name,boolean resolve)throws ClassNotFoundException{
            ...
            synchronized(getClassLoadingLock(name)){Class<?> c=findLoadedClass(name);if(c==null)try{c=findClass(name);}catch(ClassNotFoundException missing){c=getParent().loadClass(name);}if(resolve)resolveClass(c);return c;}
        }
    }
